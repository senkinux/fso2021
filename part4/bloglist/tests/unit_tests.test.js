...
describe("total likes", () => {
	...
	test("blog without likes", () => {
		const resultForBlogWithoutLikes = listHelper.totalLikes(blogWithZeroLikes)
		expect(resultForBlogWithoutLikes).toBe(0)
	})
})

describe("favorite blog", () => {
	test("return Dijkstra's blog", () => {
		const favBlog = listHelper.favoriteBlog(blogs)
		expect(favBlog).toEqual(blogs[2])
	})

	test("_id property is undefined", () => {
		const favBlog = listHelper.favoriteBlog(blogs)
		expect(favBlog._id).toBe(undefined)
	})
})

describe("most likes, most blogs", () => {
	test("most blogs", () => {
